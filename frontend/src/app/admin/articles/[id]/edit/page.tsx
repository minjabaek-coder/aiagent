...
export default function EditArticle() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [articleRes, magazinesRes] = await Promise.all([
        ...
        if (articleRes.ok) {
          ...
          setForm({
            ...
            magazineId: article.magazineId?.toString() || '',
          });
        }

        if (magazinesRes.ok) {
          const mags = await magazinesRes.json();
          setMagazines(mags);
        }
      } catch (error) {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-800">기사 수정</h1>
      </div>
